window.onload=function () {
    //页面加载    （获取多个元素，返回值为类数组 document.querySelectorAll("css选择器")）
    var lists=document.querySelectorAll('.lists');
    //smallsearch 顶部搜索栏     （获取单个元素，返回值为DOM对象  document.querySelector("css选择器")）
    ...
    window.onscroll=function () {     // onscroll	滚动条事件(滚动条位置改变)
        ...
        for(var i=0;i<dhOne.length;i++){
            //鼠标移入颜色   onmouseove鼠标移入；onmouseout	移出
    ...
    function move(){
        ...
        for (var i=0;i<bannerImg.length;i++){
            ...
            animation(bannerImg[i],{
                opacity:0,
                zIndex:0
            },500,Tween.Linear,function () {    //linear：规定以相同速度开始至结束的过渡效果
                flag=true;
            })
        }
